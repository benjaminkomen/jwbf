...
public final class JWBF {
    ...
    private static final Logger log = LoggerFactory.getLogger(JWBF.class);
    ...
    private static Map<String, String> lazyVersion() {
        ...
        URL url = urlOf(clazz, packageName); // This doesn't work when packaged as a JAR
        if (cache == null) {
    ...
    static List<ContainerEntry> jarToEntries(URI jarFileName) {
        ...
        if (jarSimpleName != null) {
            URL jarLocation1 = ClassLoader.getSystemClassLoader().getResource(jarSimpleName.toString());
            String jarLocation = jarLocation1 != null ? jarLocation1.toString() : "";
            try (JarFile jar = new JarFile(jarLocation)) {
